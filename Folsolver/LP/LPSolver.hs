...
solveWithBounds varmap bound = simplex (Minimize $ replicate (Map.size varmap) 0) (Sparse bound) []
...
solveS formulas = 
  ...
  in case (catchLeft $ mkLPBounds formulas) of
    Left err     -> error err
    Right bounds -> simplex (Minimize $ replicate (Map.size varmap) 0) (Sparse bounds) []
